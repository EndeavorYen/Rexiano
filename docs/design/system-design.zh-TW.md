...
| **Windows** | Windows 10/11 原生支援 BLE MIDI。Rexiano 透過 Web Bluetooth API 直接連接，無需額外安裝任何橋接軟體 |

> Rexiano 已實作 `BleMidiManager.ts`，透過 Web Bluetooth API 原生連接 BLE MIDI 裝置，所有平台均無需額外軟體。
