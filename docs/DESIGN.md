...
> **版本**: 1.3
> **日期**: 2026-08-25
> **狀態**: 歷史 Phase 1–9 已同步；產品下一程改為三階段（譜 → MIDI／體驗／音樂家級 MIDI → 譜）
...
8. [Phase 6 — 練習模式（已完成）](#8-phase-6--練習模式)
9. [Phase 6.5 — 兒童可用性增強（已完成）](#9-phase-65--兒童可用性增強)
10. [Phase 7 — 樂譜顯示（已完成顯示，譜尚未正確）](#10-phase-7--樂譜顯示)
11. [Phase 8 — 樂譜編輯（Extra，已完成）](#11-phase-8--樂譜編輯)
12. [Phase 9 — 打包與發佈（已完成）](#12-phase-9--打包與發佈)
13. [Synthesia 功能對照](#13-synthesia-功能對照)
14. [跨領域關注點](#14-跨領域關注點)
15. [產品下一程（三階段）](#15-產品下一程三階段)
...
工程上的歷史 Phase 1–9 已經把播放、MIDI 裝置、練習模式與五線譜**顯示**做出來。產品方向從這裡改成三個未完成階段，細節見 [§15](#15-產品下一程三階段) 與 [ROADMAP.md](./ROADMAP.md)：

| 階段 | 目標 | 明確不是 |
| ---- | ---- | -------- |
| 產品階段 1 | 譜 → MIDI，且匯入 MIDI 仍可練習（兩條入口都在） | 不是拿掉 MIDI 匯入 |
| 產品階段 2 | 更漂亮、簡潔、優雅的 UIUX，體感壓過 Synthesia | 不是再堆功能 |
| 產品階段 3 | 厲害的 MIDI → 譜，接近音樂家會寫的譜 | 不是 audio-to-score；歷史 Phase 7 ✅ ≠ 譜已正確 |
...
graph TB
    ...
    subgraph Renderer["Renderer Process (React)"]
        ...
        subgraph App["App.tsx (Router)"]
            FNC["FallingNotesCanvas (PixiJS)\nNoteRenderer · ViewportManager · tickerLoop"]
            TB["TransportBar（播放控制）"]
            PK["PianoKeyboard（88 鍵視覺化）"]
        end
    end
...
flowchart TD
    T["tickerLoop — 60 FPS"]
    S1["1. 讀取 playbackStore.currentTime"]
    S2["2. 若 isPlaying → 推進 currentTime\ndeltaMS / 1000，上限 0.1s 防跳幀"]
    S3["3. 計算可見時間範圍\n[currentTime, currentTime + windowSeconds]"]
    S4["4. 對每個 track 做 binary search\n找出可見音符"]
    S5["5. 更新 sprite 位置\n從物件池借出 / 歸還"]
    S6["6. 偵測 hit line 上的音符\n±50ms 寬容窗口 → 更新 activeNotes"]
    S7["7. 通知 React\nonActiveNotesChange callback → PianoKeyboard 高亮"]
...
graph TB
    ...
    subgraph Store["useThemeStore (Zustand)"]
        TS1["讀取 localStorage 恢復上次主題"]
        TS2["applyThemeToDOM() → 設定 :root CSS 變數"]
        TS3["hexToPixi() → PixiJS 顏色轉換"]
    end

    subgraph CSS["main.css (@theme 區塊)"]
        CSS1["定義 font-display / font-body / font-mono"]
    end

    C["各元件透過 var(--color-*) 引用"]
...
播放歌曲、觀看下落音符，並透過 Web Audio / SoundFont 同步播放鋼琴音色。
...
**狀態**: ✅ 顯示已完成；譜正確性未完成（見產品階段 3）

### 目標

在下落音符視圖之外，提供傳統五線譜顯示，讓使用者可以邊看譜邊練習。

> **完成的含義**：VexFlow 面板、游標同步、啟發式 `MidiToNotation` 已上線。
> **不是**：輸出已經是正確樂譜。量化、譜號分配、時值推斷仍是啟發式。
> 歷史 Phase 7 勾完 ≠ 譜已正確。音樂家級 MIDI → 譜是產品階段 3。
> 本節不改 `MidiToNotation` 行為。
...
graph TB
    ...
    subgraph ModeB["模式 B：僅五線譜（傳統練習）"]
        B1["五線譜（全屏，游標跟隨）"]
        B2["鍵盤"]
        B1 --> B2
    end

    subgraph ModeC["模式 C：僅下落音符（目前預設）"]
        C1["下落音符"]
        C2["鍵盤"]
        C1 --> C2
    end
...
- MusicXML 匯入 / 匯出邊界評估見 [`docs/editor-export.md`](./editor-export.md)
...
| MIDI 匯入     | ✅           | ✅                           | 已完成 | 1       |
| 下落音符      | ✅           | ✅                           | 已完成 | 2       |
| 音頻播放      | ✅ 鋼琴音色  | ✅ FreePats Upright Piano KW | 已完成 | 4 → 6.5 |
| MIDI 裝置連接 | ✅           | ✅                           | 已完成 | 5       |
| 等待模式      | ✅           | ✅                           | 已完成 | 6       |
| 速度控制      | ✅           | ✅                           | 已完成 | 6       |
| 段落循環      | ✅           | ✅                           | 已完成 | 6       |
...
| 節拍器        | ✅           | ✅                           | 已完成 | 6.5     |
| 指法建議      | ✅           | ✅                           | 已完成 | 6.5     |
| 五線譜顯示    | ❌           | ✅ 有面板；譜尚未正確         | 顯示完成 | 7；正確譜＝產品階段 3 |
| 樂譜編輯      | ❌           | ✅                           | 已完成 | 8       |
...
- **五線譜顯示**（歷史 Phase 7）：結合下落音符與五線譜，支援同步游標與顯示模式切換。這是「有譜可看」，不是「譜已正確」
- **譜 ↔ MIDI 雙向**（產品下一程）：階段 1 讓譜能練；階段 3 讓 MIDI 接近人手寫譜。都不是 audio-to-score
...
graph TD
    ...
    P5["Phase 5 — MIDI 裝置 ✅"]
    P6["Phase 6 — 練習模式 ✅"]
    P65["Phase 6.5 — 兒童可用性增強 ✅\nSprint 1: 快捷鍵/音名/SF2/暗色主題\nSprint 2: 練習面板整合/慶祝效果\nSprint 3: 成績持久化/設定面板\nSprint 4: 節拍器/新手引導/擴充曲庫\nSprint 5: 指法/洞察/中文 UI"]
    P7["Phase 7 — 樂譜顯示 ✅；有面板 ≠ 譜已正確"]
    P8["Phase 8 — 樂譜編輯 Extra ✅"]
    P9["Phase 9 — 打包發佈 ✅"]
...
- 歷史 Phase 1–9 的 roadmap 實作項目已完成（Phase 7 只保證顯示，不保證譜正確）
- 產品下一程是三個未勾選階段，見 §15 與 [ROADMAP.md](./ROADMAP.md)
- v1.0.0 已有正式 tag / GitHub Release；下一程不再用歷史 Phase 編號當產品方向

---

## 15. 產品下一程（三階段）

> 任務清單以 [ROADMAP.md](./ROADMAP.md) 為單一真實來源。這裡只寫設計界線。
> 本節不重寫引擎，也不改 `MidiToNotation` 行為。

### 15.1 產品階段 1 — 譜 → MIDI（雙入口）

把標準樂譜（優先 MusicXML）轉成可練習的 MIDI / `ParsedSong`，送進既有 Watch / Wait / Free 迴路。
...
### 15.3 產品階段 3 — 音樂家級 MIDI → 譜

把演奏 MIDI 轉成接近音樂家會寫的譜（拍子、聲部、譜號、連音、臨時記號）。

**排除：** audio-to-score。

**與歷史 Phase 7 的關係：** Phase 7 顯示層可以吃更好的 notation。有顯示 ≠ 譜正確。正確性另開實作卡驗證；在那之前不改 `MidiToNotation`。
