...
describe('noteToScreenY', () => {
  ...
  test('higher pixelsPerSecond zooms in vertically', () => {
    const yZoomed = noteToScreenY(2, vp({ currentTime: 0, pps: 400, height: 600 }))
    const yNormal = noteToScreenY(2, vp({ currentTime: 0, pps: 200, height: 600 }))
...
describe('getVisibleTimeRange', () => {
  ...
  test('window size equals canvas height divided by pps', () => {
    ...
    const [start, end] = getVisibleTimeRange(vp({ currentTime: 10, height: 600, pps: 200 }))
    expect(end - start).toBe(3)
  })

  test('higher pps means smaller time window (zoomed in)', () => {
    const [, end1] = getVisibleTimeRange(vp({ currentTime: 0, height: 600, pps: 200 }))
    const [, end2] = getVisibleTimeRange(vp({ currentTime: 0, height: 600, pps: 600 }))
