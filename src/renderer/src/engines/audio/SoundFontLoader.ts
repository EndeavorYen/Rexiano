...
export class SoundFontLoader implements ISoundFontLoader {
  ...
  async load(source: string | ArrayBuffer, audioContext: AudioContext): Promise<void> {
    ...
    if (arrayBuffer) {
      try {
        ...
        console.warn('SoundFontLoader: SF2 parsing failed, falling back to synthesis', err)
      }
    }
  ...
  private async _parseSF2(arrayBuffer: ArrayBuffer, audioContext: AudioContext): Promise<void> {
    const uint8 = new Uint8Array(arrayBuffer)
    const sf2 = new SoundFont2(uint8)

    // Collect which MIDI notes have direct samples from the SF2
    const directSamples = new Map<number, { data: Int16Array; sampleRate: number; basePitch: number }>()
    ...
    if (directSamples.size === 0) {
      for (const preset of sf2.presets) {
        ...
        for (let midi = PIANO_MIN; midi <= PIANO_MAX; midi++) {
          const keyData = sf2.getKeyData(midi, preset.header.bank, preset.header.preset)
          if (!keyData) continue
    ...
    for (const [midi, raw] of directSamples) {
      const audioBuffer = this._int16ToAudioBuffer(raw.data, raw.sampleRate, audioContext)
