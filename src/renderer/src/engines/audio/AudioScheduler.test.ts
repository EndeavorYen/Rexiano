...
function note(time: number, duration: number, midi = 60, velocity = 80): ParsedNote {
...
function song(tracks: ParsedTrack[], duration?: number): ParsedSong {
  const maxEnd = tracks.flatMap((t) => t.notes).reduce((m, n) => Math.max(m, n.time + n.duration), 0)
  return {
    fileName: 'test.mid',
...
describe('AudioScheduler', () => {
  ...
  describe('setSong', () => {
    ...
    test('subsequent setSong replaces previous song', () => {
      ...
      expect(engine.noteOn).not.toHaveBeenCalledWith(60, 80, expect.any(Number))
    })
  })
  ...
  describe('seek', () => {
    ...
    test('seek to middle skips earlier notes', () => {
      const notes = [note(0, 0.2, 60), note(0.5, 0.2, 62), note(1.5, 0.2, 64), note(2, 0.2, 65)]
      ...
      expect(engine.noteOn).not.toHaveBeenCalledWith(60, 80, expect.any(Number))
      expect(engine.noteOn).not.toHaveBeenCalledWith(62, 80, expect.any(Number))
    })
