...
function createMockMIDIPort(
  overrides: Partial<MIDIInput | MIDIOutput> = {},
...
describe("MidiDeviceManager", () => {
  ...
  describe("init", () => {
    ...
    test("sets status to denied when requestMIDIAccess rejects", async () => {
      vi.stubGlobal("navigator", {
        requestMIDIAccess: vi
          .fn()
          .mockRejectedValue(new DOMException("User denied")),
      });
      await manager.init();
    ...
    test("enumerates input and output devices on init", async () => {
      const inputPort = createMockMIDIPort({
        id: "in-1",
        type: "input",
        name: "Piano In",
      });
      const outputPort = createMockMIDIPort({
        id: "out-1",
        type: "output",
        name: "Piano Out",
      });
  ...
  describe("connect / disconnect", () => {
    ...
    beforeEach(async () => {
      inputPort = createMockMIDIPort({
        id: "in-1",
        type: "input",
        state: "connected",
      });
      outputPort = createMockMIDIPort({
        id: "out-1",
        type: "output",
        state: "connected",
      });
    ...
    test("connectInput returns false for disconnected device", async () => {
      const disconnected = createMockMIDIPort({
        id: "in-2",
        type: "input",
        state: "disconnected",
      });
    ...
    test("connectInput fires onActiveInputChange callback", () => {
      ...
      manager.connectInput("in-1");
      expect(cb).toHaveBeenCalledWith(expect.objectContaining({ id: "in-1" }));
    });
  ...
  describe("hot-plug / auto-reconnect", () => {
    test("onstatechange refreshes device lists", async () => {
      ...
      const newPort = createMockMIDIPort({
        id: "in-new",
        type: "input",
        state: "connected",
      });
    ...
    test("auto-reconnects a previously connected input device", async () => {
      ...
      (inputPort as unknown as { state: string }).state = "disconnected";
      midiAccess.onstatechange?.({} as MIDIConnectionEvent);
      expect(manager.activeInputId).toBeNull();

      // Simulate device re-plug
      (inputPort as unknown as { state: string }).state = "connected";
      midiAccess.onstatechange?.({} as MIDIConnectionEvent);
      expect(manager.activeInputId).toBe("in-1");
    });
