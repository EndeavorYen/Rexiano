...
vi.mock('pixi.js', () => {
  class MockSprite {
    x = 0; y = 0; width = 0; height = 0; visible = false; tint = 0xffffff
  }
  class MockContainer {
    children: unknown[] = []
...
let playbackStoreState = {
  ...
  isPlaying: false,
  pixelsPerSecond: 200,
  setCurrentTime: vi.fn((t: number) => { playbackStoreState.currentTime = t }),
  setPlaying: vi.fn((p: boolean) => { playbackStoreState.isPlaying = p }),
}
...
describe('FallingNotesCanvas ticker logic (via createTickerUpdate)', () => {
  ...
  test('does nothing when no song is loaded', () => {
    songStoreState = { song: null }
    const cb = vi.fn()
    const tick = createTickerUpdate(noteRenderer, () => SCREEN, { current: cb })
  ...
  test('advances currentTime when playing', () => {
    ...
    const tick = createTickerUpdate(noteRenderer, () => SCREEN, { current: undefined })
  ...
  test('skips notification when active notes have not changed', () => {
    songStoreState = { song: makeSong({
      tracks: [{
        ...
        instrument: 'Piano',
        channel: 0,
        notes: [{ midi: 60, name: 'C4', time: 0, duration: 2, velocity: 80 }],
      }],
    }) }
    playbackStoreState.currentTime = 0.5
  ...
  test('notifies when active notes transition from present to empty', () => {
    songStoreState = { song: makeSong({
      tracks: [{
        ...
        instrument: 'Piano',
        channel: 0,
        notes: [{ midi: 60, name: 'C4', time: 0, duration: 0.5, velocity: 80 }],
      }],
    }) }
  ...
  test('does not notify when no notes are at hit line (initial state unchanged)', () => {
    songStoreState = { song: makeSong({
      tracks: [{
        ...
        instrument: 'Piano',
        channel: 0,
        notes: [{ midi: 60, name: 'C4', time: 10, duration: 1, velocity: 80 }],
      }],
    }) }
