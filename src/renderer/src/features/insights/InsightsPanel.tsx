  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        />
        <StatCard
          label={t("insights.practiceTime")}
          value={formatMinutes(totalPracticeMinutes)}
        />
        <StatCard
          label={t("insights.bestAccuracy")}
          value={`${bestAccuracy.toFixed(1)}%`}
          accent
        />
        <StatCard
          label={t("insights.recentImprovement")}
  ...
  return (
    ...
    >
      ...
      <span
        ...
        style={{ color: "var(--color-text-muted)" }}
      >
        {t("insights.missRate", { rate: (spot.missRate * 100).toFixed(0) })}
      </span>
      <span
        className="text-[10px] font-mono tabular-nums w-12 text-right shrink-0"
        style={{ color: "var(--color-text-muted)" }}
      >
        {t("insights.attempts", { count: spot.totalAttempts })}
      </span>
    </div>
  );
}
