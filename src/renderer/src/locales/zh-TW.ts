/** Traditional Chinese (zh-TW) locale. */
import type { TranslationMap } from "@renderer/i18n/types";

export const zhTW: TranslationMap = {
  // ── App Shell ──────────────────────────────────────────────────
  "app.title": "Rexiano",
  "app.subtitle": "鋼琴練習",
  "app.dropMidi": "拖放 .mid 檔案到此處",
  "app.supportedFormats": "支援格式：.mid、.midi",
  "app.invalidFileType": "無效的檔案類型：{ext}。僅接受 .mid 及 .midi 檔案。",
  "app.failedParse": "無法解析 MIDI 檔案。",
  "app.failedRead": "無法讀取檔案。",
  "app.startPractice": "開始練琴",
  "app.openSettings": "設定",
  "app.menuGreeting": "今天要彈什麼呢？",
  "app.skipToContent": "跳至主要內容",

  // ── Song Library ───────────────────────────────────────────────
  "library.title": "Rexiano",
  "library.subtitle": "選一首歌開始練習吧",
  "library.searchPlaceholder": "搜尋歌曲…",
  "library.importMidi": "匯入自己的 MIDI 檔案",
  "library.noSongs": "找不到內建歌曲",
  "library.noMatch": "沒有符合篩選的歌曲",
  "library.importHint": "試試匯入一個 MIDI 檔案來開始",
  "library.difficulty.all": "全部",
  "library.difficulty.beginner": "初級",
  "library.difficulty.intermediate": "中級",
  "library.difficulty.advanced": "高級",
  "library.gradeFilter": "依等級篩選",
  "library.grade.all": "全部等級",
  "library.grade.0": "啟蒙級",
  "library.grade.1": "入門級",
  "library.grade.2": "初級",
  "library.grade.3": "基礎級",
  "library.grade.4": "初階",
  "library.grade.5": "進階初",
  "library.grade.6": "中級",
  "library.grade.7": "中高級",
  "library.grade.8": "高級",
  "library.gradeDesc.0": "啟蒙 — 3-5個音，只用右手 (C-E)",
  "library.gradeDesc.1": "入門 — 五指位置，級進 (C-G)",
  "library.gradeDesc.2": "初學 — 完整八度，簡單跳進",
  "library.gradeDesc.3": "初級 — 開始雙手，簡單低音",
  "library.gradeDesc.4": "基礎 — 雙手彈奏，I-V-I 和弦低音",
  "library.gradeDesc.5": "進階初 — 換位，輕微臨時記號",
  "library.gradeDesc.6": "中級 — 琶音，阿爾貝蒂低音，2個臨時記號",
  "library.gradeDesc.7": "中高級 — 寬音域，複雜節奏",
  "library.gradeDesc.8": "高級 — 演奏會級技巧",
  "library.greeting.morning": "早安！準備好練琴了嗎？",
  "library.greeting.afternoon": "午安！來彈首曲子吧。",
  "library.greeting.evening": "晚安！來練練琴吧。",
  "library.songPracticed": "首曲子已練習",
  "library.songsPracticed": "首曲子已練習",
  "library.session": "次練習",
  "library.sessions": "次練習",
  "library.bestScore": "最高分",
  "library.recentlyPlayed": "最近播放",
  "library.noSongsYet": "還沒有曲子",
  "library.noSongsHint": "匯入 MIDI 檔案開始練習吧！",
  "library.exploreSongs": "來探索你的第一首曲子吧！",
  "library.noMatchSearch": "沒有符合搜尋的曲子",
  "library.noMatchHint": "試試其他關鍵字或清除篩選",
  "library.emptyTitle": "開始你的鋼琴冒險吧！",
  "library.emptyHint": "拖放 MIDI 檔案到這裡，或從下方匯入",
  "library.noMatchTitle": "嗯，這裡什麼都沒有…",
  "library.noMatchAction": "試試其他關鍵字或清除篩選",
  "library.category.exercise": "\u{1F4AA} 練習",
  "library.category.popular": "\u{1F3B5} 流行",
  "library.category.holiday": "\u{1F384} 節慶",
  "library.category.classical": "\u{1F3BB} 古典",
  "library.category.all": "\u{1F4DA} 全部",

  // ── Transport Bar ──────────────────────────────────────────────
  "transport.play": "播放（空白鍵）",
  "transport.pause": "暫停（空白鍵）",
  "transport.reset": "回到開頭（Home 鍵）",
  "transport.resetLabel": "回到開頭",
  "transport.seekPosition": "播放位置",
  "transport.volume": "音量",
  "transport.mute": "靜音",
  "transport.unmute": "取消靜音",
  "transport.enableMetronome": "啟用節拍器",
  "transport.disableMetronome": "停用節拍器",

  // ── Song Header ────────────────────────────────────────────────
  "song.tracks": "軌道",
  "song.track": "軌道",
  "song.notes": "音符",
  "song.backToLibrary": "曲庫",

  // ── MIDI Device ────────────────────────────────────────────────
  "midi.noDevices": "未偵測到 MIDI 裝置",
  "midi.inputLabel": "輸入",
  "midi.outputLabel": "輸出",
  "midi.noneOption": "-- 無 --",
  "midi.disconnect": "中斷連線",
  "midi.connected": "已連線",
  "midi.disconnected": "已中斷",
  "midi.error": "連線錯誤",
  "midi.testTitle": "發送測試音符 (C4) 到選取的 MIDI 輸出",
  "midi.testLabel": "測試",
  "midi.testPlaying": "播放中…",
  "midi.testOk": "OK!",
  "midi.disconnectTitle": "中斷 MIDI 裝置連線",
  "midi.bleConnect": "連接藍牙 MIDI 裝置（如 Roland 鋼琴）",
  "midi.bleScanning": "搜尋中…",
  "midi.bleConnecting": "連線中…",
  "midi.bluetooth": "藍牙",
  "midi.bleDisconnect": "中斷藍牙 MIDI",
  "midi.bleDeviceTitle": "藍牙：{name}（點擊中斷連線）",
  "midi.testKeyboard": "測試你的鍵盤",
  "midi.pressAKey": "按一個鍵試試...",
  "midi.unsupported": "此瀏覽器不支援 Web MIDI API",
  "midi.denied": "MIDI 存取被拒絕",
  "midi.unavailable": "MIDI 存取不可用",
  "midi.initFailed": "無法初始化 MIDI 存取",

  // ── Practice Mode ──────────────────────────────────────────────
  "practice.watch": "觀看",
  "practice.wait": "等待",
  "practice.free": "自由",
  "practice.speed": "速度",
  "practice.accuracy": "準確率",
  "practice.combo": "連續正確",
  "practice.abLoop": "A–B 循環",
  "practice.setA": "將循環起點設為目前位置",
  "practice.setB": "將循環終點設為目前位置",
  "practice.clearLoop": "清除",
  "practice.more": "更多",
  "practice.showAdvanced": "顯示進階控制",
  "practice.hideAdvanced": "隱藏進階控制",
  "practice.loopSection": "循環段落",
  "practice.setALabel": "設定循環起點",
  "practice.setBLabel": "設定循環終點",
  "practice.clearLoopLabel": "清除 A-B 循環",
  "practice.encourageOnFire": "超級厲害！",
  "practice.encourageStreak": "好厲害，一直彈對！",
  "practice.encouragePerfect": "完美！",
  "practice.encourageGreat": "表現好棒！",
  "practice.encourageKeepGoing": "繼續加油！",
  "practice.encourageGettingThere": "越來越好了！",
  "practice.encourageYouCanDoIt": "你可以的！",
  "practice.scoreLabel": "練習分數",
  "practice.timingEarly": "偏早",
  "practice.timingLate": "偏晚",
  "practice.timingOnTime": "準確！",
  "practice.waiting": "等待中...",
  "practice.songComplete": "演奏完成！",
  "practice.watchComplete": "聽得很棒！",
  "practice.finalAccuracy": "最終準確率",
  "practice.bestStreak": "最佳連擊",
  "practice.complete.amazing": "太厲害了！",
  "practice.complete.great": "很棒！",
  "practice.complete.good": "不錯喔！",
  "practice.complete.tryAgain": "再試一次！",
  "practice.complete.niceListening": "聽得很棒！",
  "practice.playAgain": "再彈一次",
  "practice.backToLibrary": "回到曲庫",
  "practice.mode.watchDesc": "看和聽",
  "practice.mode.waitDesc": "音樂等你彈",
  "practice.mode.freeDesc": "自由跟著彈",
  "practice.step": "逐步",
  "practice.mode.stepDesc": "一個一個音",
  "practice.speedBumped": "加速了！現在 {speed}%",
  "practice.loop.byMeasure": "按小節",
  "practice.loop.measure": "第 {n} 小節",
  "practice.setSpeedTo": "將速度設為 {speed}",
  "practice.speedSliderLabel": "播放速度百分比",

  // ── Settings ───────────────────────────────────────────────────
  "settings.title": "設定",
  "settings.close": "關閉",
  "settings.language": "語言",
  "settings.showFingering": "顯示指法數字",
  "settings.theme": "主題",
  "settings.tab.theme": "主題",
  "settings.tab.display": "顯示",
  "settings.tab.audio": "音訊",
  "settings.tab.practice": "練習",
  "settings.tab.lang": "語言",
  "settings.tab.appearance": "外觀",
  "settings.tab.sound": "音效",
  "about.version": "版本",
  "about.changelog": "更新紀錄",
  "about.noChangelog": "尚無更新紀錄。",
  "settings.chooseTheme": "選擇主題",
  "settings.themeDesc": "主題會改變整個應用程式的顏色，包括下落音符和鍵盤。",
  "settings.displayOptions": "顯示選項",
  "settings.showNoteLabels": "在鍵盤上顯示音符名稱",
  "settings.showNoteLabelsDesc": "在鋼琴鍵上顯示音名（C、D、E…）",
  "settings.showFallingLabels": "在下落音符上顯示標籤",
  "settings.showFallingLabelsDesc": "在下落音符方塊上顯示音名",
  "settings.showFingeringDesc": "在音符上顯示建議指法數字",
  "settings.audioSettings": "音訊設定",
  "settings.volume": "音量",
  "settings.muted": "已靜音",
  "settings.muteAudio": "靜音",
  "settings.audioCompatibilityMode": "音訊相容模式",
  "settings.audioCompatibilityModeDesc":
    "使用較安全的播放延遲，改善部分 Windows 音效驅動穩定性",
  "settings.noteRelease": "音符釋放時間",
  "settings.noteReleaseMs": "{value}ms",
  "settings.practiceDefaults": "練習預設值",
  "settings.defaultMode": "預設模式",
  "settings.defaultSpeed": "預設速度",
  "settings.metronome": "節拍器",
  "settings.metronomeDesc": "播放節拍器聲音",
  "settings.countInBeats": "預備拍",
  "settings.countInOff": "關閉",
  "settings.latencyComp": "延遲補償",
  "settings.latencyDesc": "調整 MIDI 鍵盤輸入的時間偏移",
  "settings.midiChannel": "MIDI 頻道",
  "settings.midiChannelAll": "所有頻道",
  "settings.langDesc": "語言變更會立即套用到整個應用程式。",
  "settings.compactKeyLabels": "精簡鍵名標籤",
  "settings.compactKeyLabelsDesc": "在窄鍵盤上顯示較少鍵名",
  "settings.uiScale": "顯示大小",
  "settings.uiScaleDesc": "放大文字和按鈕，方便小朋友使用",
  "settings.uiScale.normal": "一般",
  "settings.uiScale.large": "大",
  "settings.uiScale.xlarge": "特大",
  "settings.kidMode": "兒童模式",
  "settings.kidModeDesc": "簡化操作介面，適合小朋友使用",

  // ── Keyboard Range ──────────────────────────────────────────────
  "toolbar.showAllKeys": "全部琴鍵",
  "toolbar.showSongKeys": "曲子範圍",

  // ── Fingering ──────────────────────────────────────────────────
  "fingering.label": "指法",

  // ── Sheet Music ────────────────────────────────────────────────
  "sheetMusic.loadSong": "載入曲子以檢視樂譜",
  "sheetMusic.modeFalling": "音符",
  "sheetMusic.modeSplit": "雙顯示",
  "sheetMusic.modeSheet": "樂譜",
  "sheetMusic.displayMode": "顯示模式",
  "sheetMusic.zoomIn": "放大",
  "sheetMusic.zoomOut": "縮小",

  // ── Audio ──────────────────────────────────────────────────────
  "audio.mute": "靜音",
  "audio.unmute": "取消靜音",
  "audio.recovering": "音訊修復中（{attempt}/{max}）…",
  "audio.restored": "音訊已恢復",
  "audio.recoveryFailed": "音訊自動修復失敗",
  "audio.retry": "重試",

  // ── Progress / Daily Goal ───────────────────────────────────
  "progress.dailyGoal": "每日目標",
  "progress.min": "分鐘",
  "progress.goalReached": "達標了！",
  "settings.dailyGoal": "每日練習目標",
  "settings.dailyGoalDesc": "設定每日練習時間來養成習慣",

  // ── Difficulty Heatmap ────────────────────────────────────────
  "heatmap.ariaLabel": "難度熱圖",
  "heatmap.easy": "簡單",
  "heatmap.medium": "中等",
  "heatmap.hard": "困難",
  "heatmap.veryHard": "非常困難",

  // ── Song Card ───────────────────────────────────────────────
  "songCard.practiced": "已練習",
  "songCard.bestScore": "最高：{score}%",
  "songCard.difficulty": "難度：{level} — {desc}",

  // ── General ────────────────────────────────────────────────────
  "general.loading": "載入中…",
  "general.error": "錯誤",
  "general.retry": "重試",
  "general.close": "關閉",
  "general.save": "儲存",
  "general.cancel": "取消",
};
