...
function getParser(store: { onNoteOn: (midi: number) => void; onNoteOff: (midi: number) => void }): MidiInputParser {
  if (!_parser) {
...
function syncParserToActiveInput(store: { onNoteOn: (midi: number) => void; onNoteOff: (midi: number) => void }): void {
  ...
  connect: async () => {
    ...
    try {
      ...
      if (manager.status === 'unsupported') {
        set({ connectionError: 'Web MIDI API is not supported in this browser' })
        return
      }
