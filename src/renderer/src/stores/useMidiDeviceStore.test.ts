...
const mockManager = {
  ...
  inputs: [] as { id: string; name: string; manufacturer: string; type: string; state: string }[],
  outputs: [] as { id: string; name: string; manufacturer: string; type: string; state: string }[],
  init: vi.fn(),
...
describe('useMidiDeviceStore', () => {
  ...
  test('connect() initializes MidiDeviceManager and sets device lists', async () => {
    mockManager.inputs = [
      { id: 'in-1', name: 'Piano', manufacturer: 'Yamaha', type: 'input', state: 'connected' },
    ]
    mockManager.outputs = [
      { id: 'out-1', name: 'Synth', manufacturer: 'Roland', type: 'output', state: 'connected' },
    ]
  ...
  test('connect() sets error when MIDI access is denied', async () => {
    ...
    expect(useMidiDeviceStore.getState().connectionError).toBe('MIDI access was denied')
  })
  ...
  test('setDeviceLists() updates inputs and outputs', () => {
    const inputs = [{ id: 'i1', name: 'KB', manufacturer: 'Y', type: 'input' as const, state: 'connected' as const }]
    const outputs = [{ id: 'o1', name: 'SP', manufacturer: 'R', type: 'output' as const, state: 'connected' as const }]
