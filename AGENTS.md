...
- 先判斷 parent issue 是否可直接驗收：若 acceptance 已被既有實作覆蓋，先跑對應驗證；證據成立時直接 close parent issue，不再拆 child。
- 若 parent issue 只差單一明確缺口，直接用 TDD 修該缺口並 commit 到 parent，commit message 使用 `Refs #N`；不要為單一缺口額外開 child issue。
- 只有在 scope 混雜、單輪無法驗收、需要多個可獨立追蹤/回復的交付物、或 parent 是 epic 時，才拆成小 issue / child issue。
...
- 不要用單一龐大 commit 混合多個無法獨立驗證的行為；若需要拆分，先記錄拆分結果，再開始寫測試。
- 合併 PR 時，Squash merge subject 需使用 release-please 可解析的 Conventional Commit 前綴，讓 CHANGELOG 不漏掉使用者可見變更：bug fix 用 `fix: ... (#N)`，新功能用 `feat: ... (#N)`，CI / 文件 / 重構分別用 `ci:` / `docs:` / `refactor:`。若 PR 真的不該進 release notes，才使用 `chore:`。
